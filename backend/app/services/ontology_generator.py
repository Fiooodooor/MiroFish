...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual natural person. When a person does not belong to any other more specific person type, use this type.
   - `Organization`: Fallback type for any organization. When an organization does not belong to any other more specific organization type, use this type.
...
- The text may contain various characters such as "primary/secondary school teacher", "passerby", "some netizen"
- If no specific type matches, they should be categorized as `Person`
- Similarly, small organizations, temporary groups, etc. should be categorized as `Organization`
...
- Person: any individual person (used when not fitting any of the above specific types)
...
- Organization: any organization (used when not fitting any of the above specific types)
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(original text has {original_length} characters; the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters have been used for ontology analysis)..."
