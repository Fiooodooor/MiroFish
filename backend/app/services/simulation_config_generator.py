...
Enables full automation without manual parameter configuration
...
CHINA_TIMEZONE_CONFIG = {
    ...
    # Night period (decreasing activity)
    "night_hours": [23],
...
class TimeSimulationConfig:
    ...
    total_simulation_hours: int = 72  # Default 72 simulation hours (3 days)
    
    # Time represented per round (simulated minutes) - default 60 minutes (1 hour), accelerates time flow
...
class PlatformConfig:
    ...
    # Echo chamber effect intensity (degree of clustering of similar views)
...
class SimulationConfigGenerator:
    ...
    and automatically generates optimal simulation parameter configurations
    ...
    def generate_config(
        ...
        reasoning_parts.append(f"Agent config: successfully generated {len(all_agent_configs)} configurations")
...
- The user group consists of Chinese users, and schedules should follow Beijing time habits
...
- **Important**: The following example values are for reference only; adjust specific time slots based on the nature of the event and participant characteristics
  - For example: student groups may peak at 21-23; media outlets are active all day; official organizations only during work hours
  - For example: breaking news may cause late-night discussion, off_peak_hours can be shortened accordingly
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            # 3. If still not found, use the agent with highest influence weight
...
- **Times should follow Chinese schedule**: almost no activity from 0-5 AM, most active from 19-22 PM
...
- **Individuals** (Student/Person/Alumni): high activity (0.6-0.9), mainly active in evenings (18-23), fast response (1-15 minutes), low influence (0.8-1.2)
