...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, used to truncate excessively long simulations)
        ...
        try:
            ...
            # If a maximum number of rounds is specified, add it to command line arguments
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create the main log file to prevent process blocking due to full stdout/stderr pipe buffers
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues where third-party libraries (e.g., OASIS) read files without specifying an encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Supported in Python 3.7+, makes all open() calls use UTF-8 by default
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set the working directory to the simulation directory (database files etc. will be generated here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create a new process group to ensure all related processes can be terminated when the server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After the process ends, do a final read of the logs
            if os.path.exists(twitter_actions_log):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the platform from the record; fall back to the default platform
    ...
    def get_all_actions(
        ...
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Automatically fill the platform field
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The old format file should have the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up the simulation's run logs (used to force a restart of the simulation)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: For dual-platform simulations, interview both platforms simultaneously and return a merged result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: For dual-platform simulations, each agent interviews both platforms simultaneously
            timeout: Timeout in seconds
